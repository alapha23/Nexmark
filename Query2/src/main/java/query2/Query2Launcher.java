...
//            LOG.info("Get query");
            // get query
            NexmarkQuery<? extends KnownSize> query =  new NexmarkQuery(configuration, new Query2(configuration));//new Query0(configuration); //getNexmarkQuery();

//            LOG.info("Create model");
            // create model
            NexmarkQueryModel model = new Query2Model(configuration); //getNexmarkQueryModel();
...
//            LOG.info("Generate events");
            // generate events
            PCollection<Event> source = p.apply("Query2" + ".ReadBounded", NexmarkUtils.batchEventsSource(configuration));
